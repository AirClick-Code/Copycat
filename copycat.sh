...
# Prompt for source directory with autocomplete
echo "Enter the path to the source directory (e.g., ~/Documents):"
echo "(Use Tab for path completion)"
read -e -r -p "> " SOURCE_DIR
...
if detect_external_drives; then
    ...
    if [[ "$choice" =~ ^[0-9]+$ ]] && [ "$choice" -ge 0 ] && [ "$choice" -lt "${#drives[@]}" ]; then
        ...
        echo "Enter a subdirectory name or press Enter to use root of $BASE_DEST_DIR:"
        echo "(Use Tab for path completion)"
        read -e -r -p "> " SUBDIR
        if [ -n "$SUBDIR" ]; then
        ...
        echo "Enter full path to the destination directory:"
        echo "(Use Tab for path completion)"
        read -e -r -p "> " DEST_DIR
    fi
else
